...
@JvmOverloads constructor(
    ...
    var viewState: ViewState = ViewState.CONTENT
        set(value) {
            ...
            if (value != previousField && getView(value) != null) {
                field = value
...
//        val inflater = LayoutInflater.from(getContext())
    ...
    fun getView(state: ViewState): View? {
        return when (state) {
            ViewState.LOADING -> getLoadingView()
            ViewState.CONTENT -> contentView
            ViewState.EMPTY -> getEmptyView()
            ViewState.ERROR -> getErrorView()
        }
    }

    private fun getLoadingView(): View? {
        return if (loadingView == null && loadingViewResId > -1) {
    ...
    private fun getErrorView(): View? {
        return if (errorView == null && errorViewResId > -1) {
            val inflater = LayoutInflater.from(context)
            val inflatedErrorView = inflater.inflate(errorViewResId, this, false)
            errorView = inflatedErrorView
            addView(inflatedErrorView, inflatedErrorView.layoutParams)
    ...
    private fun getEmptyView(): View? {
        return if (emptyView == null && emptyViewResId > -1) {
            val inflater = LayoutInflater.from(context)
            val inflatedEmptyView = inflater.inflate(emptyViewResId, this, false)
            emptyView = inflatedEmptyView
            addView(inflatedEmptyView, inflatedEmptyView.layoutParams)
    ...
    private fun setView(previousState: ViewState) {
        when (viewState) {
            ViewState.LOADING -> {
                requireNotNull(loadingView ?: getLoadingView()).apply {
            ...
            ViewState.EMPTY -> {
                requireNotNull(emptyView ?: getEmptyView()).apply {
                    contentView?.visibility = View.GONE
                    errorView?.visibility = View.GONE
            ...
            ViewState.ERROR -> {
                requireNotNull(errorView ?: getErrorView()).apply {
                    contentView?.visibility = View.GONE
                    loadingView?.visibility = View.GONE
                    emptyView?.visibility = View.GONE

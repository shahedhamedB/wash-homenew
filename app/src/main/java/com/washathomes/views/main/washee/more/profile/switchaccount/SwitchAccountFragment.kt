package com.washathomes.views.main.washee.more.profile.switchaccount
...
import com.washathomes.databinding.FragmentSwitchAccountBinding
...
//            }
            ...
            override fun onResponse(call: Call<AccountTypes>, response: Response<AccountTypes>) {
                if (response.isSuccessful){
                    if (response.body()?.results!!.size > 0){
                        Glide.with(washeeMainActivity).load(response.body()?.results?.get(0)?.icons).into(binding.washeeIcon)
                        ...
                        Glide.with(washeeMainActivity).load(response.body()?.results?.get(1)?.icons).into(binding.washerIcon)
                        ...
                        Glide.with(washeeMainActivity).load(response.body()?.results?.get(2)?.icons).into(binding.courierIcon)
    ...
    private fun checkAccount(){
        if (accountType == "1" && AppDefs.user.results?.washee_status == "1"){
            ...
            val registrationIntent = Intent(washeeMainActivity, CourierRegistrationActivity::class.java)
            startActivity(registrationIntent)
            washeeMainActivity.finish()
        }
    }
    ...
    private fun saveUserToSharedPreferences() {
        ...
        when (accountType) {
            ...
            "2" -> {
                val mainIntent = Intent(washeeMainActivity, WasherMainActivity::class.java)
                startActivity(mainIntent)
            }
            "3" -> {
                val mainIntent = Intent(washeeMainActivity, CourierMainActivity::class.java)
                startActivity(mainIntent)
            }
        }
        washeeMainActivity.finish()

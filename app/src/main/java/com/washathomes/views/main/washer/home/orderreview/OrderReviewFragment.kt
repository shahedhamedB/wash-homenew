package com.washathomes.views.main.washer.home.orderreview
...
import com.washathomes.databinding.FragmentOrderReviewBinding
...
class OrderReviewFragment : Fragment() {
    ...
    lateinit var review: WasherReviewInfo
    ...
    override fun onCreateView(
        ...
        val layout = inflater.inflate(R.layout.fragment_order_review, container, false)
        binding = FragmentOrderReviewBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun setData(){
        ...
        if (review.washee_info.image.isNotEmpty()){
            Glide.with(washerMainActivity).load(review.washee_info.image).into(binding.imageOne)
        }
        binding.questionOne.text = resources.getString(R.string.how_was)+" "+review.washee_info.name

        if (review.courier_info != null){
            if (review.courier_info!!.image.isNotEmpty()){
                Glide.with(washerMainActivity).load(review.courier_info!!.image).into(binding.imageTwo)
            }
    ...
    private fun getReview(){
        ...
        val categoriesCall: Call<WasherReview> =
            retrofit.create(RetrofitAPIs::class.java).getWasherReview(orderId)
        categoriesCall.enqueue(object : Callback<WasherReview> {
            override fun onResponse(call: Call<WasherReview>, response: Response<WasherReview>) {
            ...
            override fun onFailure(call: Call<WasherReview>, t: Throwable) {
                binding.progressBar.visibility = View.GONE
    ...
    private fun submitReview(){
        ...
        val categoriesCall: Call<BooleanResponse> =
            retrofit.create(RetrofitAPIs::class.java).washerOrderReview(review)
        categoriesCall.enqueue(object : Callback<BooleanResponse> {
            override fun onResponse(call: Call<BooleanResponse>, response: Response<BooleanResponse>) {
                binding.progressBar.visibility = View.GONE
                if (response.isSuccessful){
                    Toast.makeText(washerMainActivity, resources.getString(R.string.reviews_success), Toast.LENGTH_SHORT).show()
                    val intent = Intent(washerMainActivity, WasherMainActivity:: class.java)

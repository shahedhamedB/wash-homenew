...
import com.washathomes.databinding.FragmentOrderPlacedOverviewBinding
...
class OrderPlacedOverviewFragment : Fragment() {
    ...
    lateinit var binding: FragmentOrderPlacedOverviewBinding
    ...
    override fun onCreateView(
        ...
        val layout = inflater.inflate(R.layout.fragment_order_placed_overview, container, false)
        binding = FragmentOrderPlacedOverviewBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initViews(view: View){
        ...
        washerPending = args.washerPending
    }
    ...
    private fun onClick(){
        ...
        binding.viewItemsLayout.setOnClickListener { navController.navigate(OrderPlacedOverviewFragmentDirections.actionOrderPlacedOverviewFragmentToWasherViewItemsFragment(true)) }
    }
    ...
    private fun acceptOrder(){
        ...
        val ordersCall: Call<BooleanResponse> =
            retrofit.create(RetrofitAPIs::class.java).washerAcceptOrder(orderObj)
        ordersCall.enqueue(object : Callback<BooleanResponse> {
            override fun onResponse(call: Call<BooleanResponse>, response: Response<BooleanResponse>) {
                binding.progressBar.visibility = View.GONE
                if (response.isSuccessful){
                    Toast.makeText(washerMainActivity, resources.getString(R.string.order_accepted), Toast.LENGTH_SHORT).show()
    ...
    private fun rejectOrder(){
        ...
        val retrofit: Retrofit = Retrofit.Builder().baseUrl(Urls.BASE_URL).client(okHttpClient)
            .addConverterFactory(GsonConverterFactory.create()).build()
        val ordersCall: Call<BooleanResponse> =
            retrofit.create(RetrofitAPIs::class.java).washerRejectOrder(orderObj)
        ordersCall.enqueue(object : Callback<BooleanResponse> {
            override fun onResponse(call: Call<BooleanResponse>, response: Response<BooleanResponse>) {
                binding.progressBar.visibility = View.GONE
                if (response.isSuccessful){
                    Toast.makeText(washerMainActivity, resources.getString(R.string.order_rejected), Toast.LENGTH_SHORT).show()
                    navController.popBackStack()
                }else{
                    val gson = Gson()
                    val type = object : TypeToken<ErrorResponse>() {}.type //ErrorResponse is the data class that matches the error response
